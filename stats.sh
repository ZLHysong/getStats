...
vnstat --oneline | cut -d ';' -f4 # today's received
vnstat --oneline | cut -d ';' -f5 # today's transmitted
...
top -bn1 | grep load | awk '{printf "%.2f\n", $(NF-2)}' >> log.txt
vnstat --oneline | cut -d ';' -f4  >> log.txt # today's received
vnstat --oneline | cut -d ';' -f5  >> log.txt # today's transmitted
