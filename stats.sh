...
df -h | grep /dev/vda1 | awk '{printf "%.2f ", $(NF-2)/$(NF-3)}'
...
df -h | grep /dev/vda1 | awk '{printf "%.2f ", $(NF-2)/$(NF-3)}' >> log.txt
top -bn1 | grep load | awk '{printf "%.2f ", $(NF-2)}' >> log.txt
vnstat --oneline | cut -d ';' -f4 | awk '{printf "%.2f ", $(NF-1)}' >> log.txt # today's received
